    ...
    PathResult PathFinder::find_path(const PathFindingParams &params)
    {
        ...
        while (!heap.empty() && iterations < max_iter)
        {
            ...
            if (current.node_id == params.sink)
            {
                ...
                std::vector<RRNodeId> nodes_rev;
                nodes_rev.push_back(node);
                while (parent_edge.count(node) > 0 && parent_edge[node].is_valid())
                {
                    result.edges.insert(result.edges.begin(), parent_edge[node]);
                    node = parent_node[node];
                    nodes_rev.push_back(node);
                }
                // nodes_rev está em ordem [sink, ..., driver] — inverter
                result.nodes.assign(nodes_rev.rbegin(), nodes_rev.rend());
